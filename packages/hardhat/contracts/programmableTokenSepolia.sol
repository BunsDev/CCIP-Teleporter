...
// Usage of Chainlink's CCIP and Avalanche's Teleporter exemple contracts
...
import {SenderOnChain} from "./SenderOnChain.sol";
/**
 * THIS IS BASED ON AN EXAMPLE CONTRACT THAT USED HARDCODED VALUES FOR CLARITY.
 * THIS IS AN EXAMPLE CONTRACT THAT USES UN-AUDITED CODE.
 * SOME VALUES ARE STILL HARDCODED FOR CLARITY AND EASE OF USE / TEST.
...
contract ProgrammableTokenTransfers is CCIPReceiver, OwnerIsCreator {
    ...
    address senderTeleporter_address = address(0);
    address receiverTeleporter_address = address(0);

    function set_SenderOnChain (address _address) external onlyOwner{
        senderTeleporter_address = _address;
    }
    
    function set_ReceiverOnDispatch (address _address) external onlyOwner{
        receiverTeleporter_address = _address;
    }
    ...
    /// @param _recipient final recipient.
    ...
    /// @param _destinationChainSelector The identifier (aka selector) for the destination blockchain.
    /// @param _receiver The address of the recipient on the destination blockchain.
    /// @param _recipient The final recipient.
    /// @param _token token address.
    /// @param _amount token amount.
    /// @return messageId The ID of the CCIP message that was sent.
        ...
        ) // Make sure source chain and sender are allowlisted
    {
        ...
        //Handle the creation of the teleporter message
        //add recipient address
        address recipient = stringToAddress(s_lastReceivedText);
        bytes memory message = abi.encode(
            s_lastReceivedTokenAmount,
            recipient,
            s_lastReceivedTokenAddress
        );
        SenderOnChain senderTeleporter = SenderOnChain(senderTeleporter_address);
        //trigger the teleporter
        senderTeleporter.sendMessage(receiverTeleporter_address, message);
    ...
    function _buildCCIPMessage(
        ...
        return
            Client.EVM2AnyMessage({
                ...
                data: abi.encode(toString(_recipient)), // ABI-encoded string
